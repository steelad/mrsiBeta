function [F,J] = ...
    funjacoMRSI(alf,dummy,stof,t,demptype,gausstype,freqtype,etype,...
...
% param is a matrix of size L x (m*K) where L is the number of signals (e.g. grid of 16x16 => L = 256 signals),  K is the number of metabolites, m is the number of parameters per metabolite (e.g. for Lorentzian model, m=4 since there are 4 parameters: amplitude, frequency, damping and phase).
%   The way the m*K parameters are sorted is identical to the one used in init_one_run.m (first the amplitudes, then the dampings and so on).
%   E.g.: param(i,j) will be the value of parameter j from signal i. 
% BETA:
% beta is an L-vector of boolean values: 
% beta(i)=true if current voxel is of the same type and neighbor to voxel i
%        =false, otherwise
...
% size(param,2) should be nrvar; temporary test:
if (size(param,2)~=nrvar)
    error('param matrix has wrong dimensions');
end
% tempor test W
if (size(Weightm,1)~=nrvar)
    error('W size not correct')
end
...
F1=norm(F);
for i = 1:L
    F2(i)=norm(Weightm*(alf-param(i,:))');
    if F2(i)==0, F2(i)= 1; end
    if beta(i)
        F = [F;.1*F1/F2(i)*Weightm*(alf-param(i,:))'];
    end
end
