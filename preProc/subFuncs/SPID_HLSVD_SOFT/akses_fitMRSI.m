function [recon,baseline,filrecon,filbaseline,filoriginal,...
          ...
          residual] = ...
    akses_fitMRSI(invivosignal,dbase,t,step,abs_scl_factor,...
              ...
              param,beta,weightfordamp,weightforgauss,weightforfreq,sigma)
...
%   param = a matrix of size L x (nrbasis*nrvar) where L is the number of
%           signals (e.g. for a grid of 16x16 => L = 256 signals),
%             nrbasis=size(dbase,2) is the number of metabolites,
%             nrvar is the number of parameters per metabolite,
%            containing prior parameter values for the MRSI grid
%   beta = an L-vector of boolean values: 
%          beta(i)=true if current voxel is of the same type and neighbor 
%           to voxel i
%          beta(i)=false, otherwise
%  weightfordamp,weightforgauss,weightforfreq = scalar weights for 
%          penalizing the distance between parameters of neighboring voxels
...
for i = 1:L
    if beta(i)
        dataz = [dataz;zeros(length(alf),1)];
     end
end
W = init_weightingMRSI(demp,nra,nrd,nrg,nrf,nrph,nre,...
...
[x,resnorm,residual,some_exitflag,some_output,some_lambda,J]  = ...
    lsqcurvefit('funjacoMRSI',alf,1,dataz,lb,ub,options,dbase,t,...
                ...
                baseline_information,step,plotting,param,beta,W);
