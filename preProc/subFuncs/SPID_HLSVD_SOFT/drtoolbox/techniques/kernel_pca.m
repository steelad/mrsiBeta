...
% Maastricht University, 2007
    ...
    if size(X, 1) < 3000
        % Get Gram matrix for training points
        disp('Computing kernel matrix...'); 
        K = gram(X, X, kernel, param1, param2);

        % Normalize kernel matrix K
        D = sum(K) / ell;           % column sums
        E = sum(D) / ell;           % total sum
        J = ones(ell, 1) * D;       % column sums (in matrix)
        ...
        options.isreal = 1;
        options.issym = 1;
        [V, L] = eigs(@(v)kernel_function(v, X', 1, kernel, param1, param2), size(X, 1), no_dims, 'LM', options);       
    end
    ...
    % Set feature vectors in original order
    mappedX = mappedX';
