...
% Maastricht University, 2007
    ...
    net_size = (d * layer1_size + layer1_size * layer2_size + layer2_size * layer3_size + layer3_size * layer4_size);     % size of individual
    ...
    for i=1:no_gen
        ...
        for j=1:no_ind
            ...
            ind_b = 1; ind_e = d * layer1_size;
                w1 = weights(j, ind_b:ind_e);
                w1 = reshape(w1, [d layer1_size]);
            ind_b = ind_e + 1; ind_e = ind_e + layer1_size * layer2_size;
                w2 = weights(j, ind_b:ind_e);
                w2 = reshape(w2, [layer1_size layer2_size]);
            ind_b = ind_e + 1; ind_e = ind_e + layer2_size * layer3_size;
                w3 = weights(j, ind_b:ind_e);
                w3 = reshape(w3, [layer2_size layer3_size]);
            ind_b = ind_e + 1; ind_e = ind_e + layer3_size * layer4_size;
                w4 = weights(j, ind_b:ind_e);
                w4 = reshape(w4, [layer3_size layer4_size]);
            w5 = w4';
            w6 = w3';
            w7 = w2';
            w8 = w1';
