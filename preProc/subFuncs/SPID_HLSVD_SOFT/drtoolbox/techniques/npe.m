...
% Maastricht University, 2007
    ...
    if size(X, 1) > 1500 && no_dims < (size(X, 1) / 10)
        if strcmp(eig_impl, 'JDQR')
            ...
            [eigvector, eigvalue] = jdqz(WP, DP, no_dims, 'LA', options);
        else
            options.disp = 0;
            options.issym = 1;
            options.isreal = 0;
